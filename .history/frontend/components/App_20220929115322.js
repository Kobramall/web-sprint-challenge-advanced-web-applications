...
export default function App() {
  ...
  const getArticles = () => {
    ...
    .then(res => 
             {setArticles(res.data.articles), loading(false)}      
    )
    .catch(err => console.error(err))
  
    props.setMessage(`Here are your articles, ${props.user.username}!`)
  }
  ...
  const deleteArticle = article_id => {
    ...
    .then(()=> getArticles())
    setMessage(`Article ${article_id} was deleted, ${user.username}!`)
  }
  ...
  return (
    ...
    <>
      ...
      <div id="wrapper" style={{ opacity: spinnerOn ? "0.25" : "1" }}> {/* <-- do not change this line */}
        ...
        <Routes>
          <Route path="/" element={<LoginForm login={login} user={user} setUser={setUser} setMessage={setMessage}/>} />
          <Route element ={<PrivateRoute />}>
             <Route path="articles"  element ={
               <>
                 ...
                 <Articles values={values} setValues={setValues} getArticles={getArticles} articles={articles} setIsEditing={setIsEditing} deleteArticle={deleteArticle} isEditing={isEditing} setMessage={setMessage} user={user} />
               </>}/>
          </Route>
        </Routes>
